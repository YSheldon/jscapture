...
Contacts.prototype = {
    ...
    init: function (strId, option, existeduserlist) {
        ...
        _this.dataType=_this.option.data[0].type;
        ...
        if ($(".selec_title", strId).length == 0) {
            ...
            <li class="Winclose" ></li>
              ...
              <em></em><input  type="text" class="search" placeholder="" value="搜索"><em style="display: none" class="del_seach"></em>
              </div>
               <div class="contacts_leftContent" style="display: block">
               <ul class="contact_tab">
                   <li class="contact_type contact_Select ">`+_this.option['data'][0]['tab']+ `</li>
...
<div style="width: 100%;text-align: center" class="btn_container">
    ...
    _init: function (strId, existeduserlist) {
        ...
        if (typeof(existeduserlist) != 'undefined' && existeduserlist.length > 0) {
            ...
            for (var i = 0, ln = existeduserlist.length; i < ln; i++) {
                ...
                var name = XssToString(curuserinfo.name);
    ...
    bindEvent: function (strId) {
        ...
        $('.Winmin', strId).on('click', function () {
        ...
        $('.Winmax', strId).on('click', function () {
        ...
        document.onkeydown=function(event){
            keyDown($('#contacts_person_scroll'),'ml_bg',_this.Scrollleft)
                        ...
                        if (_this.zTree == null) {
                            ...
                            for (var i = 0; i < childlist.length; i++) {
                                ...
                                if (curNode.isParent == false) {
                                    ...
                                    var parm = {resourceList: [{photoResId: curNode['imagpath']}],size:96};
    ...
    createTree: function (zNodes) {
        ...
        function bindselected(treeNode) {
            ...
            if (treeNode.isParent) {
                str += '<li nodeid="' + id + '" orgid="' + treeNode['orgid'] + '" userUniId="' + treeNode['orgid'] + '" tId="' + tid + '" isstruct="true" pId="' + treeNode.pId + '"><img src="./images/list_ic_org.png" alt="" ><span>' + XssToString(strName) + '</span><em></em></li>'
            } else {
                str += '<li nodeid="' + id + '" orgid="' + treeNode['orgid'] + '" userUniId="' + treeNode['userUniId'] + '" tId="' + tid + '"  name="' + strName + '" pId="' + treeNode.pId + '"><img src="' + strImgePath + '" alt="" ><span>' + XssToString(strName) + '</span><em></em></li>'
            }
                ...
                if (curorgStruct['type'] == 1) { //成员
                    for (var j = 0; j < orgMemberList.length; j++) {
                        ...
                        if (curorgMember['id'] == curorgStruct['orgMemberId']) {
                            ...
                            if(curorgMember.visibleFlag==2){//隐藏成员
                                continue;
                            }
    ...
    ConfirmContact: function (strId, callback) {
        ...
        $('.contacts_right .btnOK').click(function () {
            ...
            $(".SelecedContact li", strId).each(function (index, ele) {
                ...
                var strName = $ele.children().eq(1).text();
                ...
                if ($ele.attr('isstruct')) {
                    ...
                    if(strName.indexOf('外')>-1){
                        obj = {userUniId: userUniId, tId: tId, ImgePath: strImgePath, name: strName, isstruct: false,type:1}
                    }else{
                    ...
                    if (status == 0) {
                        ...
                        for (var m = 0; m < list.length; m++) {
                            ...
                            for (var n = 0; n < curgroup.length; n++) {
                                ...
                                var sName=XssToString(curPerson.name)
                                if (sName == '' || sName == null || typeof(sName) == 'undefined') {
                                    sName = '未知'
                                }
                                else if (curPerson.contactDomain == curPerson.recordDomain) {
                                    sName = sName
                                } else {
                                    sName = sName + '<i class="contact_info">外</i>'
                                }
                                ...
                                str += '<li userUniId="' + curPerson.contactUniId + '"><em class=" aV_check ' + status + '" ></em><img photoResId="' + curPerson.photoResId + '" src="./images/persion.png" alt=""><span >' + sName + '</span><span title="' + branch + '">' + XssToString(branch) + '</span></li>'
                            }
                            str += '</ul>'
                        }
                        str += '</div>';
    ...
    showQunMember:function(strId){
        if (this.ContactPerson == null) {
            $('.Contact_load', strId).show();
            var _this = this;

                    var parm={dialogId:_this.option.data[0].dialogId}//查询用户的ID（如果不传此字段，就返回这个群的所有用户信息）”｝


            try {
                window.lxpc.exebusinessaction('VideoConf', 'GetQunMemberInfo', '0', JSON.stringify(parm), 0, function (status, jsondata, targ) {
                    $('.Contact_load', strId).hide();
...
var str='<ul>'
                                for (var n = 0; n < datalist.length; n++) {
                                    ...
                                    str += '<li userUniId="' + curPerson.userUniId + '"><em class=" aV_check ' + status + '" ></em><img photoResId="' + curPerson.photoResId + '" src="./images/persion.png" alt=""  ><span >' + XssToString(sName) + '</span><span title="' + branch + '">' + XssToString(branch) + '</span></li>'
                                }
                                str += '</ul>'
                        ...
                        _this.ContactPerson = datalist.sort(function (a, b) {
                            return a.abbreviation.localeCompare(b.abbreviation)
                        });
    ...
    bindContactPer:function($li){
        ...
        $('#contacts_person_scroll li').removeClass('ml_bg');
...
var userId=$(this).parents('li').attr('userUniId')
                $(this).parent().remove();

                $('#contacts_person_scroll li[userUniId="' + userId + '"] em').removeClass('aV_checkEd')
    ...
    SearchContact: function (search) {
        ...
        try {
            ...
            window.lxpc.exebusinessaction('Notice', 'SearchContact', '0', JSON.stringify(parm), _this.searchId, function (status, datalist, targ) {
                    ...
                    if (PersonContacts.length > 0) {
                        ...
                        for (var m = 0; m < PersonContacts.length; m++) {
                            ...
                            if (aperson.contactDomain == aperson.recordDomain) {
                                sName = XssToString(aperson.name)
                            } else {
                                sName = XssToString(aperson.name) + '<i class="contact_info">外</i>'
                            }
                            var checkstatus = ''
                            ...
                            str += '<li userUniId="' + aperson['contactUniId'] + '"><em class="aV_check ' + checkstatus + '" ></em><img photoResId="' + aperson.photoResId + '" src="./images/persion.png" alt=""><span >' + sName + '</span><span>' + XssToString(aperson.branchPath) + '</span></li>'
                        }
                        str += '</ul>'
                    ...
                    $('.contacts_search_scroll li').on('click', function () {
                        ...
                        if (!$em.hasClass('aV_checkEd')) {
                            ...
                            $('.SelecedContact li:last em').click(function () {
                                ...
                                $('.contacts_search_con li[userUniId="' + _this.memberList[index] + '"] em ').removeClass('aV_checkEd');
    ...
    getHeadImg: function ($imgs) {
        ...
        $imgs.each(function (index, ele) {
            ...
            _this.getImgePath(parm, _this.imageId, function (result, targ) {
                ...
                var $img=$imgs.filter('[imageId="' + targ + '"]').attr('src', result);
