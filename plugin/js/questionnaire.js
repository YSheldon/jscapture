...
//点击选择题-选项 显示可编辑
    ...
    uploadImg: function () {
        ...
        $(".aV_edit_img").click(function (e) {
            ...
            _this.uploadfile(function (file) {
                ...
                $ele.parents(".aV_t_editOpa").append('<div class="aV_queImg"><span class="preview"></span><a href="javascript:;" class="aV_a aV_queImg_del " onclick="imgDel(this)">删除</a></div>');
    ...
    addQuestion: function () {
        ...
        var newtext = ''
        if ($('.aV_question >li').length == 0) {
            newtext = 'Q1: '
        } else {
            var lasttext = $('.aV_question >li:last').find('h4').html();

            newtext = 'Q' + (parseInt(lasttext.substring(1, lasttext.length - 1)) + 1) + ':'
        }
        ...
        var $last = $(str).appendTo('.aV_question')

        _this.scroll_send.refresh();
        _this.scroll_send.scrollToElement($last[0])
        //var $ele = $('.aV_question >li:last ').find('.aV_queCon');
        var $ele = $last.find('.aV_queCon');

        //$('.aV_question').append(str);
        //_this.scroll_send.refresh();
        //var $ele = $('.aV_question >li:last ').find('.aV_queCon'),
        var $title = $ele.find('.aV_issue_title');
        _this.leftBtn($ele)
    ...
    addSingle: function () {
        ...
        //var $last=   $('.aV_question').append(str);
        var $last = $(str).appendTo('.aV_question')
...
//点击选项-图片右侧的删除，删除图片
var imgDel=function(ele){
        $(ele).parents(".aV_queImg").remove();
}
...
var WriteQuest = function (curNotice, callback) {
    ...
    var id = curNotice.noticeEntity.id,
        ...
        realName = XssToString(curNotice.noticeEntity.realName);
    ...
    try {
        ...
        window.lxpc.exebusinessaction('Notice', 'QueryNoticeDetail', '0', JSON.stringify(parm), 1, function (status, jsondata, targ) {
            ...
            if (status == 0) {
                ...
                for (var i = 0, ln = questlist.length; i < ln; i++) {
                    var item = questlist[i],
                        ...
                        content = 'Q' + (i + 1) + ': ' + XssToString(item.content)
                        ...
                        if (voteLimit == 1) {//单选题
                            ...
                            for (var j = 0, ln1 = optionList.length; j < ln1; j++) {
                                ...
                                var optItem = optionList[j],
                                    optcontent = XssToString(optItem.content),
                                    optId = optItem.id,
                                    joinCount = optItem.joinCount,
                                    resId = optItem.resId,
                                mimeType=optItem.mimetype;
                                ...
                                if (resId == '' || typeof(resId) == 'undefined') {
                                    ...
                                    str += `<li><a href="javascript:;" class="aV_radio " optId="${optId}" Limit="${voteLimit}"></a>
                                        <span>${(j + 1) + ': ' + optcontent}</span><img src="./images/defpicture.png" resId="${resId}" mimeType="${mimeType}" alt="${optcontent}"></li>`
                                }
                            ...
                            for (var j = 0, ln1 = optionList.length; j < ln1; j++) {
                                ...
                                if (resId == '' || typeof(resId) == 'undefined') {
                                    ...
                                    str += ` <li >
                                        <a href="javascript:;" class="aV_check " optId="${optId}" ></a>
                                        <span>${(j + 1) + ': ' + optcontent}</span><img src="./images/defpicture.png" alt="${optcontent}" mimeType="${mimeType}" resId="${resId}">
                                    </li>`
                                }
                            }
                            str += `</ul></div>`
                ...
                $('.content_Re .opt_lis img').each(function (index, ele) {
                    ...
                    _this.imageList.push({resId: resId, mineType: mimeType})
    ...
    try {
        ...
        window.lxpc.exebusinessaction('Notice', 'QueryNoticeDetail', '0', JSON.stringify(parm), 1, function (status, jsondata, targ) {
            ...
            if (status == 0) {
                ...
                if (targ == 0) {// targ==0第一次请求resID不存在的时候发起第二次请求通知详情 //targ=1 获取通知详情来得到调查问卷的详细情况
                    if(targ==0){
                        var data = JSON.parse(jsondata);
                        _this.bindAttachment(curNotice, data)
                        return;
                ...
                var data = JSON.parse(jsondata),
                    ...
                    realName = XssToString(data.noticeEntity.realName),
                ...
                for (var i = 0, ln = questlist.length; i < ln; i++) {
                    var item = questlist[i],
                        type = item.type,
                        voteLimit = item.voteLimit,
                        content=XssToString(item.content);

                     content = 'Q' + (i + 1) + ': ' + content
                    ...
                    if (type == 1) {//选择题
                        ...
                        for (var j = 0, ln1 = optionList.length; j < ln1; j++) {
                            ...
                            optcontent=XssToString(optcontent)
                            var     opttitle = (j + 1) + ': ' + optcontent;
                            ...
                            if (resId != '' && typeof(resId) != 'undefined') {
                                ...
                                str += `<img src="./images/defpicture.png" alt="sdasdaserwer" resId="${resId}" mineType="${optItem.resourceMimeType}">`
                            }
                ...
                function bindPicture(imglist, ismyselfSend) {
                    ...
                    imglist.each(function (index, ele) {
                        ...
                        _this.imageId = _this.imageId + 1;
                        $ele.attr('targ', _this.imageId);
                        var mineType= $ele.attr('mineType')
                        var parm = {resourceList: [{photoResId: resId}], size: 96};

                        _this.imageList.push({resId: resId, mineType: mineType})
...
//显示问答题的信息
    function getQuestAnswer(data, recordDomain) {
        ...
        document.getElementById('ques_tip').innerHTML = XssToString(data.content)
    ...
    function getImgs() {
        ...
        $('.Quest_con img').each(function () {
            ...
            var mimeType = $img.attr('mimeType')
        ...
        QueryNoticeMember(data.joinCount, parm, NoticeId, recordDomain, function (datalist) {
            ...
            for (var i = 0, ln = datalist.length; i < ln; i++) {
                ...
                str += `<li style="overflow: hidden;zoom: 1"><img src="./images/defuser.png" alt="" resId="${curData.photoResId}">
                    <span>${XssToString(curData.name)}</span>
                    <span class="fr Quest_con_t">${time}</span>
                <p>${XssToString(curData.branchPath)}</p>
                </li>
                `
            }
            str += '</ul>'
